...
def attack(model, X, source_, target_, device, token_signature,\
    ...
    for t in tqdm(range(iterations), desc="Attack"):
        ...
        target_scores = output2[:,source_]
        masked_output2=output2.clone()
        masked_output2[:,source_]=float("-inf")
        other_max_scores, _ =masked_output2.max(dim=1)
        score_diff=target_scores-other_max_scores
        masked_score_diff = score_diff.clone()
        masked_score_diff[masked_score_diff >= LOWER_BOUNDRY_TARGET] = float('-inf')
        max_vals_conf,max_vals_conf_ins = torch.topk(masked_score_diff, k=2, dim=0)
        s_indices = ((max_labels_1[max_vals_conf_ins] == source_) & (LOWER_BOUNDRY_TARGET >= max_vals_conf) & (~nan_rows[max_vals_conf_ins])).nonzero()
    ...
    if k_to_use > 0:
        ...
        if "4x" in model_name:
            ...
            x3 = F.relu(model.fc3(x2))
            x4 = F.relu(model.fc4(x3))
            x5 = F.relu(model.fc4(x4))
            x6 = F.relu(model.fc4(x5))
            x7 = F.relu(model.fc4(x6))
            x8 = F.relu(model.fc4(x7))
            x9 = F.relu(model.fc4(x8))
            layers_outputs.append(torch.mean(torch.sign(x1), dim=0))
            layers_outputs.append(torch.mean(torch.sign(x2), dim=0))
            layers_outputs.append(torch.mean(torch.sign(x3), dim=0))
            layers_outputs.append(torch.mean(torch.sign(x4), dim=0))
            layers_outputs.append(torch.mean(torch.sign(x5), dim=0))
            layers_outputs.append(torch.mean(torch.sign(x6), dim=0))
            layers_outputs.append(torch.mean(torch.sign(x7), dim=0))
            layers_outputs.append(torch.mean(torch.sign(x8), dim=0))
            layers_outputs.append(torch.mean(torch.sign(x9), dim=0))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--lower_boundery_for_target', type=float, default=0, help='lower_boundery_for_target')
    ...
    LOWER_BOUNDRY_TARGET = args.lower_boundery_for_target
