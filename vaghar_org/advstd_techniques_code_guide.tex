...
through $N_2$ alone, with a single zonotope seeded from the input
box $[0,1]^n$ (\src{source\_b\_seed\_box}). Every encoder bounds the
perturbed input to the same box, so the one hull is exact for
\emph{both} copies. Initialisation:
$\mathbf c^{\mathrm{B}}_0 = \tfrac12\mathbf 1$,
$G^{\mathrm{B}}_0 = \mathrm{diag}(\tfrac12)$
