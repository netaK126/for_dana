...
function add_random_noise_to_dict!(dict::Dict, e::Float32)
    total_noise = Dict()
    for (key, matrix) in dict
        ...
        noise_sum = sum(noise)
        total_noise[key] = noise_sum
        dict[key] .= matrix .+ noise
    end
    return total_noise
end

function get_nn(model_path, model_name, w, h, k, c, dataset, weight_addition)
...
#         fc3 = get_matrix_params_mod(dict, "fc3", (layers_n, c),weight_addition)
    ...
    return nn, total_noise
end


function get_nn_variables_opt(model_path, model_name, w, h, k, c, dataset, weight_addition)
    ...
    if model_name == "3x10"
        ...
        dict = Dict{String,Any}( "fc1/weight"=>model_pth[1],"fc1/bias" => reshape(model_pth[2],(1,length(model_pth[2]))),
             "fc2/weight"=>model_pth[3],"fc2/bias" => reshape(model_pth[4],(1,length(model_pth[4]))),
            "fc3/weight"=>model_pth[5],"fc3/bias" => reshape(model_pth[6],(1,length(model_pth[6]))))
        fc1_o = get_matrix_params(dict, "fc1", (w*h*k, layers_n))
        fc1_m = get_matrix_params(dict, "fc1", (w*h*k, layers_n))
        fc2_o = get_matrix_params(dict, "fc2", (layers_n, layers_n))
        fc2_m = get_matrix_params(dict, "fc2", (layers_n, layers_n))
        fc3_o = get_matrix_params(dict, "fc3", (layers_n, c))
        fc3_m = get_matrix_params_mod(dict, "fc3", (layers_n, c),weight_addition)
        nn = Sequential( [ Flatten([1, 3, 2, 4]),fc1_o, ReLU(), fc2_o, ReLU(), ], "nn",)
        last_layer_o = Sequential( [fc3_o], "nn",) #Sequential([fc3_o],"fc3_o")
        last_layer_m = Sequential( [ fc3_m], "nn",) #Sequential([fc3_m],"fc3_m")
    elseif model_name == "cnn0"
        is_conv = true
        stride_to_use_1 = 4
        stride_to_use_2 = 4
    ...
    return nn, last_layer_o, last_layer_m
end