...
function save_results(results_path, model_name, perturbation, results_str, d, nn, ss, w_, h_, k_, type_of_problem,itr,opt_version, weight_addition_str , is_deps, is_encode_deps_neta)
    opt_version_str = ""
    if opt_version
        opt_version_str="_LessBoolians"
    end
    constraints_str = ""
    if is_deps
        constraints_str = "_addingVagharConstraints"
    end
    if is_encode_deps_neta
        constraints_str = constraints_str * "_addingNetaConstraints"
    end

    file = open(results_path*model_name * "_itr"* itr*"_"*type_of_problem*"ObjectiveDiff_differentNetworks"*weight_addition_str*constraints_str*opt_version_str*".txt", "w")
