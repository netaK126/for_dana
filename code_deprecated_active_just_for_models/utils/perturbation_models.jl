function get_model(w_, h_, k_,
    ...
    elseif perturbation == "diff"
        return merge(d_common, get_diff_keys(nn, nn1, nn2, input, m,opt_version))
...
function get_diff_keys(nn::NeuralNet, nn1::NeuralNet, nn2::NeuralNet, input::Array{<:Real}, m::Model,opt_version::Bool,)::Dict{Symbol,Any}
    input_range = CartesianIndices(size(input))
    v_in = map( i -> @variable(m, lower_bound = 0, upper_bound = 1), input_range,)
    if opt_version
        common = v_in |> nn
        v_output_nn1= common |> nn1
        v_output_nn2= common |> nn2
    else
        v_output_nn1= v_in |> nn1
        v_output_nn2= v_in |> nn2
    end
    return Dict(:v_output_nn1 => v_output_nn1, :v_in => v_in, :v_output_nn2 => v_output_nn2)
end
