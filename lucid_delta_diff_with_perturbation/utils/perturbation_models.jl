function get_model(
    ...
    if perturbation == "brightness"
        return merge(d_common, get_perturbation_specific_keys_brightness(perturbation_size, nn, nn_hyper, input, m))
...
function get_perturbation_specific_keys_brightness(perturbation_size, nn::NeuralNet, nn_hyper::NeuralNet,input::Array{<:Real}, m::Model,)::Dict{Symbol,Any}
    global network_version
    global layer_counter
    global nueron_counter
    global all_bounds_of_original
    ...
    input_range = CartesianIndices(size(input))
    p_size = perturbation_size[1]
    v_e = map(_ -> @variable(m, lower_bound = 0, upper_bound = perturbation_size), input_range,)
    v_in = map(i -> @variable(m, lower_bound = 0, upper_bound = 1), input_range,)
    v_x0 = map(i -> @variable(m, lower_bound = 0, upper_bound = 1+perturbation_size), input_range,)
    @constraint(m, v_x0 .== v_in + v_e)
