...
function dep_additional(m, layers_n, layer, phi_dep, phi_dep_l, perturbation, perturbation_size, activation_cnt, activation_function_num)
    ...
    elseif (perturbation == "translation")
        #TBD May 2024 for geometric case
    end
    phi_dep = phi_dep_l
    return phi_dep
end
    ...
    for n in 1:size(phi_dep_c)[1]
        ...
        if haskey(layers_info_dict,(activation_cnt+activation_function_num*layers_n,n)) && haskey(layers_info_dict,(activation_cnt+(activation_function_num+1)*layers_n,n))
            ...
            if dep == 0
                ...
                @constraint(m,av[ind_o+2]==av[ind_p+2])
            elseif dep == 1
                println("dep 1 adds" * str(av[ind_o+1]) *" " * str(av[ind_p+1] ))
                @constraint(m,av[ind_o+1]<=av[ind_p+1])
                @constraint(m,av[ind_o+2]<=av[ind_p+2])
            elseif dep == -1
                println("dep -1 adds" * str(av[ind_o+1]) *" " * str(av[ind_p+1] ))
                ...
                if reuse_bounds_conf.is_reuse_bounds_and_deps == false
                    ...
                    elseif l_p>=u_o
                        ...
                        if (l_diff != Inf) & (l_diff>-non_equality_tolerance)
                            println("(l_diff != Inf) & (l_diff>-non_equality_tolerance)")
                            @constraint(m,av[ind_o+1]>=av[ind_p+1])
                            @constraint(m,av[ind_o+2]>=av[ind_p+2])
                            phi_dep_c[n] = -1
                        end
                        if u_diff<non_equality_tolerance
                            println("u_diff<non_equality_tolerance")
                            @constraint(m,av[ind_o+1]<=av[ind_p+1])
                            @constraint(m,av[ind_o+2]<=av[ind_p+2])
                            phi_dep_c[n] = 1
                        end
